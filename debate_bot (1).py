...
ADMIN_IDS     = [int(x) for x in os.environ.get("ADMIN_IDS", "7968570881").split(",")]
...
async def su_confirm(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if t == tr(l,"ok"):
        ...
        ids_to_delete = [i for i in range(max(mid-80,0), mid+1) if i != thanks.message_id]
        await asyncio.gather(*[c.bot.delete_message(chat_id=cid, message_id=i) for i in ids_to_delete], return_exceptions=True)
